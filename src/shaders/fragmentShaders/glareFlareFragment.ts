...
export const glareFlareFragment = tgpu['~unstable'].fragmentFn({
  ...
  //offsets
  let center_offset = d.vec2f(-0.2, -0.1);
  let offsetted_center = std.add(center, center_offset);
  ...
  let angle = std.atan2(
    std.add(rot.y + center_offset.y, uv_norm.y),
    std.sub(rot.x + center_offset.x, uv_norm.x)
  );
  ...
  // Offsetting params
  let depth_factor = std.mul(std.add(center_offset, d.vec2f(dist)), 0.0);

  dist = std.distance(
    std.add(offsetted_center, std.length(depth_factor)),
    centeredCoords
  );
  dist = std.clamp(dist, 0.0, 1.0);
  angle = std.atan2(
    std.add(rot.y + center_offset.y /** depth_factor.x*/, uv_norm.y),
    std.sub(rot.x + center_offset.x /** depth_factor.y*/, uv_norm.x)
  );
...
export const glareFlareParabolicFragment = tgpu['~unstable'].fragmentFn({
  in: { uv: d.vec2f },
  out: d.vec4f,
})((input) => {
  const uv = d.vec2f(input.uv.x, 1.0 - input.uv.y);
  ...
  //offsets
  let center_offset = d.vec2f(-0.6, 0.0);
  let offsetted_center = std.add(center, center_offset);

  offsetted_center = d.vec2f(
    offsetted_center.x * offsetted_center.x,
    2.2 * offsetted_center.y * offsetted_center.x
  );
  ...
  // Calculate distance from center and ensure it does not exceed 1
  let angle = std.atan2(
    std.add(offsetted_center.y, uv_norm.y),
    std.sub(offsetted_center.x, uv_norm.x)
  );

  dist = std.distance(offsetted_center, centeredCoords);
  dist = std.clamp(dist, 0.0, 1.0);
  // angle = std.atan2(
  //   std.add(rot.y + center_offset.y /** depth_factor.x*/, uv_norm.y),
  //   std.sub(rot.x + center_offset.x /** depth_factor.y*/, uv_norm.x)
